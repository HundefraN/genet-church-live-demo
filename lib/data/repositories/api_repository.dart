...
class ApiRepository {
  ...
  Future<PaginatedResult<Member>> getMembers(String churchId,
      {int page = 1, int limit = 20}) async {
    final response = await _dio.get('/church-members', queryParameters: {
      'churchId': churchId,
      'page': page,
      'limit': limit,
    });
    ...
    final members = responseData.map((json) => Member.fromJson(json)).toList();
  ...
  Future<List<Pastor>> getPastors({int page = 1, int limit = 100}) async {
    final response = await _dio.get('/pastors', queryParameters: {
      'page': page,
      'limit': limit,
    });
    final responseData = response.data['data'] as List;
  ...
  Future<Pastor> addPastor(
    ...
    final response = await _dio.post('/pastors',
  ...
  Future<Pastor> updatePastor(
    ...
    final response = await _dio
        .patch('/pastors/$pastorId', data: {'fullName': fullName, 'email': email});
    return Pastor.fromJson(response.data);
  }
  ...
  Future<Pastor> assignChurchToPastor(
    ...
    final response = await _dio.patch('/pastors/$pastorId/assign-church',
        data: {'churchId': churchId});
    return Pastor.fromJson(response.data);
  }

  Future<List<Church>> getChurches({int page = 1, int limit = 100}) async {
    final response = await _dio.get('/churches', queryParameters: {
      'page': page,
      'limit': limit,
    });
    final responseData = response.data['data'] as List;
  ...
  Future<Church> updateChurch(
    ...
    final response =
    await _dio.patch('/churches/$churchId', data: church.toUpdateJson());
    return Church.fromJson(response.data);
  }
  ...
  Future<List<Department>> getDepartments(String churchId) async {
    final response =
    await _dio.get('/departments', queryParameters: {'churchId': churchId});
    final responseData = response.data as List;
    return responseData.map((json) => Department.fromJson(json)).toList();
  }

  Future<Department> addDepartment(
      {required String name, required String churchId}) async {
    final response = await _dio.post('/departments',
        data: {'name': name}, queryParameters: {'churchId': churchId});
  ...
  Future<Department> updateDepartment(
    ...
    final response =
    await _dio.patch('/departments/$departmentId', data: {'name': name});
    return Department.fromJson(response.data);
  }
  ...
  Future<Servant> assignServantToDepartment(
      {required String departmentId, required String servantId}) async {
    final response = await _dio.post('/departments/$departmentId/assign-servant',
        data: {'servantId': servantId});
    return Servant.fromJson(response.data);
  }

  Future<List<Servant>> getServants(String churchId,
      {int page = 1, int limit = 100}) async {
    final response = await _dio.get('/servants', queryParameters: {
      'churchId': churchId,
      'page': page,
      'limit': limit,
    });
  ...
  Future<Servant> addServant(
    ...
    final response = await _dio.post('/servants',
        data: {'fullName': fullName, 'email': email, 'password': password},
        queryParameters: {'churchId': churchId});
    return Servant.fromJson(response.data);
  }
  ...
  Future<Servant> updateServant(
    ...
    final response =
    await _dio.patch('/servants/$servantId', data: {'fullName': fullName});
    return Servant.fromJson(response.data);
  }
  ...
  Future<List<Session>> getSessions() async {
    ...
    final sessionsList = (allSessionsResponse.data as List)
        .map((json) =>
        Session.fromJson(json, currentSessionId: currentSessionId))
        .toList();
